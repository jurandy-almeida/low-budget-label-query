...
from datasets import Perturbate, Repeat, Sample, Subset
...
def main():
    parser = argparse.ArgumentParser(description='PyTorch CoDIAL + UNFOLD OfficeHome')
    parser.add_argument('--root', '-r', type=PathType(exists=True, type='dir'),
                        metavar='PATH', default='data/OfficeHomeDataset_10072016',
                        help='path to the OfficeHome dataset')
    parser.add_argument('--source', '-s', metavar='DATA', default='Art',
                        choices=dataset_names,
                        help='image datasets: ' + ' | '.join(dataset_names) +
                        ' (default: Art)')
    parser.add_argument('--target', '-t', metavar='DATA', default='Clipart',
                        choices=dataset_names,
                        help='image datasets: ' + ' | '.join(dataset_names) +
                        ' (default: Clipart)')
    ...
    if args.source == 'Art':
        source_train_data = Repeat(datasets.ImageFolder(root=os.path.join(args.root, args.source), transform=transforms.Compose(source_train_transform)), repeats=2)
    else:
    ...
    if not os.path.isfile(source_model_file):
        if args.mode == 'dual':
            ...
            model_weights = cluster.fit(source_train_data, Repeat(target_train_data, repeats=2) if args.target == 'Art' else target_train_data, target_val_data)
            ...
            model_weights = cluster.fit(source_train_data, Repeat(perturb_target_train_data, repeats=2) if args.target == 'Art' else perturb_target_train_data, target_val_data)
